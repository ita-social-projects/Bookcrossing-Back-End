...
namespace Application.Services.Implementation
{
    public class SuggestionMessageService : ISuggestionMessageService
    {
        ...
        public async Task<List<SuggestionMessageDto>> GetAll()
        {
            return _mapper.Map<List<SuggestionMessageDto>>(await _messageRepository.GetAll().AsNoTracking().Include(u => u.User).ThenInclude(r => r.Role).ToListAsync());
        }

        public async Task<PaginationDto<SuggestionMessageDto>> GetAll(FullPaginationQueryParams fullPaginationQuery)
        {   
            var query = _messageRepository.GetAll().AsNoTracking().Include(u => u.User).ThenInclude(r => r.Role);
        ...
        {
            return _mapper.Map<SuggestionMessageDto>(await _messageRepository.GetAll().AsNoTracking().Include(u => u.User).ThenInclude(r => r.Role).AsNoTracking().FirstOrDefaultAsync(m => m.Id == messageId));
        }
